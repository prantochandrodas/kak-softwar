    ...
    <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
        ...
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item text-muted"> {{ __('messages.fund_receive_list') }} </li>
                <!--end::Item-->
            </ul>
            <!--end::Breadcrumb-->
        </div>
                ...
                <table id="featuredProjectTitleHeading" class="display" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('messages.from_fund') }}</th>
                            <th>{{ __('messages.from') }} {{ __('messages.bank') }}</th>
                            <th>{{ __('messages.from') }} {{ __('messages.account') }}</th>

                            <th>{{ __('messages.to_branch') }}</th>
                            <th>{{ __('messages.to_fund') }}</th>
                            <th>{{ __('messages.to') }} {{ __('messages.bank') }}</th>
                            <th>{{ __('messages.to') }} {{ __('messages.account') }}</th>
...
<script>
    ...
    $(document).ready(function() {
        table = $('#featuredProjectTitleHeading').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('fund.receive-list.getdata') }}',
                data: function(d) {
    ...
    $(document).on('click', '.receive', function() {
        ...
        $.ajax({
            url: "/admin/fund-transfer/receive/" + id,
            type: "POST",
            data: {
            ...
            success: function(res) {
                ...
                $('#featuredProjectTitleHeading').DataTable().ajax.reload();
            },
    ...
    $(document).on('click', '.return', function() {
        ...
        $.ajax({
            url: "/admin/fund-transfer/return/" + id,
            type: "POST",
            data: {
                _token: $('meta[name="csrf-token"]').attr('content')
            },
