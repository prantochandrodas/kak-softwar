    ...
    <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
        ...
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item text-muted">{{ __('messages.bank_branch_list') }}</li>
                <!--end::Item-->
            </ul>
            <!--end::Breadcrumb-->
        </div>
...
@include('backend.bank_branch.create')
@endsection
@push('script')
...
<script>
    $(document).ready(function() {
        $('#featuredProjectTitleHeading').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('bank-branch.getdata') }}',
            columns: [{
    ...
    $(document).on('click', '.edit', function() {
        ...
        $.ajax({
            url: '/admin/bank-branch/edit/' + dataId,
            type: 'GET',
    ...
    $(document).on('click', '.toggle-status', function() {
        var id = $(this).data('id');
        $.ajax({
            url: '{{ route('bank-branch.toggleStatus') }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                id: id
