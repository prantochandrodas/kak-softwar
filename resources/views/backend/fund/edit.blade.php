...
<div class="modal-header" style="background-color: #333333; border-bottom: 1px solid #ccc;">
    <h5 class="modal-title" id="dataEditModalLabel" style="color: #ffffff;"> {{ __('messages.edit_fund') }}</h5>
...
<script>
    $(document).ready(function() {
        ...
        $("#dataEditModalForm").on("submit", function(e) {
            ...
            $.ajax({
                url: "{{ route('fund.update', ':id') }}".replace(':id', sliderId),
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        if (errors.opening_balance) {
                            $(".opening-balance-error").text(errors.opening_balance[0])
                                .show();
                        }
