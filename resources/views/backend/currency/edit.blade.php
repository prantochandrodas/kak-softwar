...
<div class="modal-header" style="background-color: #333333; border-bottom: 1px solid #ccc;">
    <h5 class="modal-title" id="dataEditModalLabel" style="color: #ffffff;"> {{ __('messages.edit_currency') }}</h5>
...
<div class="modal-body">
    <form id="dataEditModalForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <!-- symbole -->
            <div class="col-md-12 mb-3">
                <label for="symbole" class="form-label fw-bold text-dark">
                    {{ __('messages.symbole') }}:</label>
                <input type="text" class="form-control border-dark" id="symbole" name="symbole"
                    placeholder="{{ __('messages.symbole') }}" value="{{ $data->symbole }}">
                <div class="invalid-feedback symbole-error"></div>
            </div>
            <!-- rate -->
            <div class="col-md-12 mb-3">
                <label for="rate" class="form-label fw-bold text-dark">
                    {{ __('messages.rate') }}:</label>
                <input type="number" class="form-control border-dark" id="rate" name="rate"
                    placeholder="{{ __('messages.rate') }}" step="any" value="{{ $data->rate }}">
                <div class="invalid-feedback rate-error"></div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $("#dataEditModalForm").on("submit", function(e) {
            ...
            $.ajax({
                url: "{{ route('currency.update', ':id') }}".replace(':id', sliderId),
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href =
                            "{{ route('currency.index') }}?added-successfully=" +
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        if (errors.rate) {
                            $(".rate-error").text(errors.rate[0]).show();
                        }
                        if (errors.symbole) {
                            $(".symbole-error").text(errors.symbole[0]).show();
                        }
