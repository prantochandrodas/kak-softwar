    ...
    <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
        ...
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item text-muted"> {{ __('messages.sub_category_list') }}</li>
                <!--end::Item-->
            </ul>
            <!--end::Breadcrumb-->
        </div>
...
<div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        ...
        <button data-bs-toggle="modal" data-bs-target="#dataCreateModal" class="btn btn-sm btn-success mb-2"><svg
            ...
            </svg> {{ __('messages.create_category') }}
        </button>
        <table id="featuredProjectTitleHeading" class="display" style="width:100%">
            <thead>
                <tr>
                    <th>{{ __('messages.serial_no') }}</th>
                    <th>{{ __('messages.category') }} {{ __('messages.name') }}</th>
                    <th>{{ __('messages.sub_category') }} {{ __('messages.name') }}</th>
...
@include('backend.sub_category.create')
@endsection
@push('script')
...
<script>
    $(document).ready(function() {
        $('#featuredProjectTitleHeading').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('sub-category.getdata') }}',
            columns: [{
    ...
    $(document).on('click', '.edit', function() {
        ...
        $.ajax({
            url: '/admin/sub-category/edit/' + dataId,
            type: 'GET',
    ...
    $(document).on('click', '.toggle-status', function() {
        var id = $(this).data('id');
        $.ajax({
            url: '{{ route('sub-category.toggleStatus') }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                id: id
