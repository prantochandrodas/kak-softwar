...
<div class="modal-header" style="background-color: #333333; border-bottom: 1px solid #ccc;">
    <h5 class="modal-title" id="dataEditModalLabel" style="color: #ffffff;">{{ __('messages.edit_sub_category') }}</h5>
    ...
    <form id="dataEditModalForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-md-12 mb-3">
                <label for="category_id" class="form-label fw-bold text-dark">{{ __('messages.category') }}:
            ...
            <!-- name -->
            <div class="col-md-12 mb-3">
                <label for="name" class="form-label fw-bold text-dark">{{ __('messages.category') }}
                    {{ __('messages.name') }}:</label>
                <input type="text" class="form-control border-dark" id="name" name="name"
                    placeholder="{{ __('messages.category') }} {{ __('messages.name') }}" value="{{ $data->name }}">
...
<script>
    $(document).ready(function() {
        ...
        $("#dataEditModalForm").on("submit", function(e) {
            ...
            $.ajax({
                url: "{{ route('sub-category.update', ':id') }}".replace(':id', sliderId),
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href =
                            "{{ route('sub-category.index') }}?added-successfully=" +
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        if (errors.category_id) {
                            $(".category-id-error").text(errors.category_id[0]).show();
                        }
