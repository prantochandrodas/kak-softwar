...
<div class="modal-header" style="background-color: #333333; border-bottom: 1px solid #ccc;">
    <h5 class="modal-title" id="dataEditModalLabel" style="color: #ffffff;"> {{ __('messages.edit_bank') }}</h5>
                ...
                <div class="mb-3">
                    ...
                    <select name="fund_id" id="fund_id" class="form-select">
                        ...
                        @foreach ($funds as $item)
                            <option value="{{ $item->id }}" {{ $data->fund_id == $item->id ? 'selected' : '' }}>
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
...
<script>
    $(document).ready(function() {
        ...
        $("#dataEditModalForm").on("submit", function(e) {
            ...
            $.ajax({
                url: "{{ route('bank.update', ':id') }}".replace(':id', sliderId),
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        if (errors.fund_id) {
                            $(".fund-error").text(errors.fund_id[0])
                                .show();
                        }
