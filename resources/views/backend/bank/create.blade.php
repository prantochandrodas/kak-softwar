                ...
                <form id="dataCreateModalForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            <label for="name" class="form-label fw-bold text-dark">{{ __('messages.bank') }}
                                {{ __('messages.name') }}:</label>
                            <input type="text" class="form-control border-dark" id="name" name="name"
                                placeholder="{{ __('messages.bank') }} {{ __('messages.name') }}">
...
<script>
    $(document).ready(function() {
        $("#dataCreateModalForm").on("submit", function(e) {
            ...
            $.ajax({
                url: "{{ route('bank.store') }}", // Laravel route for storing data
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href =
                            "{{ route('bank.index') }}?added-successfully=" +
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        if (errors.fund_id) {
                            $(".fund-error").text(errors.fund_id[0]).show();
                        }
