...
class BankController extends Controller
{
    ...
    {
        if (!check_access("bank.list")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
        $funds = Fund::where('status', 1)->get();
        return view('backend.bank.index', compact('funds'));
    }

    public function getdata(Request $request)
    {
        if ($request->ajax()) {
            $data = Bank::with('fund')->orderBy('created_at', 'desc')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $deleteUrl = route('bank.distroy', $row->id);
                    $csrfToken = csrf_field();
                    $method = method_field('DELETE');
                    if (check_access("bank.edit")) {
                ...
                ->rawColumns(['action', 'status', 'fund'])
                ->make(true);
        }
    }
    public function store(Request $request)
    {
        if (!check_access("bank.create")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
    ...
    public function edit($id)
    {
        ...
        $funds = Fund::where('status', 1)->get();
        return view('backend.bank.edit', compact('data', 'funds'));
    }
