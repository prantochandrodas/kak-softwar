...
class SubCategoryController extends Controller
{
    ...
    {
        if (!check_access("sub_category.list")) {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $deleteUrl = route('sub-category.distroy', $row->id);
                    $csrfToken = csrf_field();
                    $method = method_field('DELETE');
                    if (check_access("sub_category.edit")) {
                    ...
                    if (check_access("sub_category.status")) {
    ...
    {
        if (!check_access("sub_category.create")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
    ...
    {
        if (!check_access("sub_category.delete")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
        try {
            ...
            DB::commit();
            Alert::success('Delete Success', 'Delete Successfully');
            return redirect()->back();
        } catch (\Exception $exception) {
            DB::rollBack();
            Alert::error('Delete Failed', 'Something went wrong! Please try again.');
            return redirect()->back();
        }
    }
}
