...
class DesignationController extends Controller
{
    ...
    {
        if (!check_access("designation.list")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
        $departments = Department::where('status', 1)->get();
        return view('backend.designation.index', compact('departments'));
    }

    public function getdata(Request $request)
    {
        if ($request->ajax()) {
            $data = Designation::with('department')->orderBy('created_at', 'desc')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $deleteUrl = route('designation.distroy', $row->id);
                    $csrfToken = csrf_field();
                    $method = method_field('DELETE');
                    if (check_access("designation.edit")) {
                    ...
                    if (check_access("designation.status")) {
    ...
    {
        if (!check_access("designation.create")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
    ...
    public function edit($id)
    {
        ...
        return view('backend.designation.edit', compact('data', 'departments'));
    }
