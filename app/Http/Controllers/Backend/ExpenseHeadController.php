...
class ExpenseHeadController extends Controller
{
    ...
    {
        $heads = ExpenseHead::where('branch_id', $id)
            ->where('status', 1)
            ->get();
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $action = '';
                    $toggleIcon = '';
                    $editUrl = route('expense-head.edit', $row->id);
                    $deleteUrl = route('expense-head.distroy', $row->id);
                    ...
                    if (check_access("expense-head.status")) {
    ...
    public function edit($id)
    {
        ...
        $data = ExpenseHead::findOrFail($id);
    ...
    public function toggleStatus(Request $request)
    {
        ...
        $department = ExpenseHead::find($request->id);
        if ($department) {
