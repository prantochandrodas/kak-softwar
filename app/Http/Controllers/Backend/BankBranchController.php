...
class BankBranchController extends Controller
{
    ...
    {
        if (!check_access("bank_branch.list")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
        $banks = Bank::where('status', 1)->get();
        return view('backend.bank_branch.index', compact('banks'));
    }

    public function getdata(Request $request)
    {
        if ($request->ajax()) {
            $data = BankBranch::with('bank')->orderBy('created_at', 'desc')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $deleteUrl = route('branch.distroy', $row->id);
                    $csrfToken = csrf_field();
                    $method = method_field('DELETE');
                    if (check_access("bank_branch.edit")) {
                    ...
                    if (check_access("bank_branch.status")) {
    ...
    {
        if (!check_access("bank_branch.create")) {
            Alert::error('Error', "You don't have permission!");
            return redirect()->route('admin.dashboard');
        }
    ...
    public function edit($id)
    {
        ...
        $data = BankBranch::findOrFail($id);
        $banks = Bank::where('status', 1)->get();
        return view('backend.bank_branch.edit', compact('data', 'banks'));
    }
    ...
    public function toggleStatus(Request $request)
    {
        ...
        $data = BankBranch::find($request->id);
        if ($data) {
